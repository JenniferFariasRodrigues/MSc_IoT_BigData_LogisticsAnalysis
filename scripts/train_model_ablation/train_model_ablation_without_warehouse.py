"""
train_model_ablation_without_warehouse.py
--------------------------------------------------
Author: Jennifer Farias Rodrigues
Date: 24/03/2025
Description: Train XGBoost model without 'warehouse_inventory_level' to evaluate feature impact.
...
FEATURE_TO_REMOVE = "warehouse_inventory_level"
...
# Remove unsupported types for XGBoost (e.g., datetime, object)
unsupported_types = ['datetime64[ns]', 'object']
cols_to_remove = [col for col in features.columns if features[col].dtype.name in unsupported_types]
features = features.drop(columns=cols_to_remove)

print("Removed columns with unsupported types:", cols_to_remove)
